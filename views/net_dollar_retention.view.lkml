view: net_dollar_retention {
  derived_table: {
    sql: WITH all_months AS (
      ...
      -- take running average ^ 12
      monthly_avg_annualized_over_time AS (
        SELECT
          *
          , POW(AVG(net_dollar_retention) OVER (ORDER BY month), 12) AS avg_annualized_net_dollar_retention
        FROM monthly_avg_annualized_dollar_net_retention
        WHERE month >= '2021-01-01'
        ORDER BY month
      )

      SELECT * FROM monthly_avg_annualized_over_time
  ...
  measure: avg_annualized_net_dollar_retention {
    type: sum
    sql: ${TABLE}.avg_annualized_net_dollar_retention ;;
    value_format: "0.00%"
  }

  set: detail {
    fields: [month, net_dollar_retention, avg_annualized_net_dollar_retention]
  }
}
